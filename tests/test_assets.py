...
class TestAssetManager:
    ...
    def test_find_asset_rejects_absolute_path(self, temp_assets_dir: Path) -> None:
        """Test that absolute paths are rejected."""
        manager = AssetManager([temp_assets_dir])
        with pytest.raises(AssetNotFoundError, match="absolute"):
            manager.find_asset("/etc/passwd")

    def test_find_asset_rejects_traversal(self, temp_assets_dir: Path) -> None:
        """Test that path traversal sequences are rejected."""
        manager = AssetManager([temp_assets_dir])
        with pytest.raises(AssetNotFoundError, match="not allowed"):
            manager.find_asset("../../../etc/passwd")
