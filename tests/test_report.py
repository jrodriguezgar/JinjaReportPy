...
class TestSection:
    ...
    def test_empty_template(self) -> None:
        ...
        section = Section(name="empty", template="")
        assert section.render() == ""

    def test_section_renders_with_custom_filters(self) -> None:
        """Custom Jinja2 filters (currency, format_date, etc.) must work in sections."""
        section = Section(
            name="filters",
        ...
        assert "1" in html
        assert "234" in html
        # Ensure it didn't fall back to raw output
...
class TestReport:
    ...
    def test_report_css_collection(self) -> None:
        ...
        # render_css() returns page/section CSS only
        css = report.render_css()
        assert ".custom" in css

        # global_css is included by render() in the full HTML
        html = report.render()
